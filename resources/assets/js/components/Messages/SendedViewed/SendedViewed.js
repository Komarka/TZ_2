...
import './SendedViewed.css';
import { Link } from 'react-router';
const SendedViewed=(props)=>{
...
let my_image=`http://social/img/users/${from_id}.jpeg?${Math.random()}`;
  ...
  <img className="media-object" src={my_image} width='30px' height='40px' />
    <p className="extra text-viewed">{message}</p></div>
    </div>
  </Link>
</div>
  )
